...
const adminNavItems: NavItem[] = [
  { id: 'admin-dashboard', label: 'Dashboard', icon: LayoutDashboard, section: 'Admin Kecamatan' },
  { id: 'school-management', label: 'Data Sekolah', icon: School, section: 'Admin Kecamatan' },
  { id: 'operator-applicants', label: 'Data Pendaftar', icon: Users, section: 'Admin Kecamatan' },
  { id: 'quota-management', label: 'Kuota', icon: PieChart, section: 'Admin Kecamatan' },
  { id: 'schedule-management', label: 'Jadwal', icon: Calendar, section: 'Admin Kecamatan' },
  { id: 'requirement-management', label: 'Jalur & Syarat', icon: Route, section: 'Admin Kecamatan' },
  { id: 'prompt-ai-management', label: 'Prompt AI', icon: MessageSquare, section: 'Admin Kecamatan' },
  { id: 'chat-ai-settings', label: 'Pengaturan Chat AI', icon: Settings, section: 'Admin Kecamatan' },
  { id: 'announcement-management', label: 'Pengumuman', icon: Megaphone, section: 'Admin Kecamatan' },
  { id: 'rekap', label: 'Rekap', icon: MapPin, section: 'Admin Kecamatan' },
  { id: 'operator-account', label: 'Akun Petugas', icon: UserCog, section: 'Admin Kecamatan' },
];
...
export function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const getDynamicPageTitle = (): string => {
    ...
    return adminNavItems.find((item) => item.id === currentPage)?.label || 'Admin';
  };

  const pageTitle = getDynamicPageTitle();

  // Group nav items by section
  const sections = adminNavItems.reduce<Record<string, NavItem[]>>((acc, item) => {
        ...
        <div className="flex items-center gap-3 px-3 mb-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-white/50 truncate">
              {currentUser?.email || 'admin@spmb.kec.id'}
            </p>
          </div>
        </div>
        <button
